...
const Application = () => {
  ...
  const filterData = () => {
    ...
    const filteredData = appList.filter((item: any) => {
      const searchMatch =
        !search ||
        keysToSearch.some((key: any) => item[key].toLowerCase().includes(search.toLowerCase()));
      const dateMatch =
        date.length === 0 ||
        (new Date(item.date) >= new Date(date[0]) && new Date(item.date) <= new Date(date[1]));
        ...
        Cell: (appList: any) => (
          <>
            <div className="d-flex align-items-center">
              ...
              <div className="flex-grow-1 ms-2 ">{appList.row.original.company}</div>
            </div>
          </>
        ),
      },
                ...
                <CardBody className="pt-0">
                  <div>
                    <Nav className="nav nav-tabs nav-tabs-custom nav-success mb-3" role="tablist">
                      <NavItem>
                        <NavLink
                          className={classnames({ active: activeTab === 'All' }, 'All py-3')}
                          onClick={() => {
                            toggleTab('All');
                          }}
                        >
                      ...
                      <NavItem>
                        <NavLink
                          className={classnames({ active: activeTab === 'New' }, 'New py-3')}
                          onClick={() => {
                            toggleTab('New');
                          }}
                        >
